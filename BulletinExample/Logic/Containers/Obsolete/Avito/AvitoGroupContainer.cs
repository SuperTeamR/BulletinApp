...
namespace BulletinExample.Logic.Containers.Avito
{
    ...
    internal class AvitoGroupContainer : GroupContainerBase
    {
        ...
        ///
        /// <returns>   Сигнатура группы </returns>
        ///-------------------------------------------------------------------------------------------------
        ...
        public override void Reinitialize()
        {
            DCT.Execute(data =>
            {
                var fieldParser = FieldParserContainerList.Get(Uid);
                    ...
                    foreach(var f in group.Fields)
                    {
                        ...
                        if(dbField == null)
                        {
                            dbField = new FieldTemplate
                            {
                                ...
                                Tag = f.Value.Tag,
                                IsImage = fieldParser.ImageFields.Contains(f.Key),
                                IsDynamic = fieldParser.DynamicSelects.Contains(f.Key),
                                LastChangeId = null,
                                HasRemoved = false,
                            };
                        ...
                        if (f.Value.Options != null && f.Value.Options.Length > 0)
                        {
                            ...
                            {
                                var dbGroupedFieldId = dbGroupedField.Id;
                                var dbOption = data.Db1.SelectOptions.FirstOrDefault(q => q.GroupedFieldId == dbGroupedFieldId && q.Code == o.Value);

                                if (dbOption == null)
                                {
                                    dbOption = new SelectOption
                                    {
                                        Code = o.Value,
                                        GroupedFieldId = dbGroupedFieldId,
                                        Name = o.Text
                                    };
