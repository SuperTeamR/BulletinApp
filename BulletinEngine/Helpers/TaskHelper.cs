...
namespace BulletinHub.Tools
{
    public static class TaskHelper
    {
        ...
        static void ClearOldTasks(Guid userId)
        {
            BCT.Execute(d =>
            {
                var tasks = d.TempDB.Tasks.Where(q => q.UserId == userId).ToArray();
                d.TempDB.Tasks.RemoveRange(tasks);
                d.SaveChanges();
            });
        }
        ...
        static void AddNewBulletinTasks(Guid userId)
        {
            BCT.Execute(d =>
            {
               ...
               if (bulletins.Length == 0) return;
                ...
                var accesses = d.BulletinDb.Accesses.Where(q => q.UserId == userId && q.State != (int)DefaultState.Enable).ToArray();
